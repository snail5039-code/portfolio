...
export default function WeatherCard({ weather, loading }: { weather: WeatherResponse; loading: boolean }) {
  ...
  return (
    <section aria-label="현재 날씨" className="rounded-2xl border border-sky-100 bg-gradient-to-br from-sky-50 via-white to-blue-50/50 p-3.5">
      ...
      <div className="mt-3 grid grid-cols-1 gap-2 text-[11px] text-slate-600 min-[360px]:grid-cols-2"><span className="flex min-w-0 items-center gap-1.5 rounded-lg bg-white/80 px-2 py-1.5 ring-1 ring-sky-100/70"><Droplets size={13} className="shrink-0 text-blue-500" /><span className="truncate">강수 {current.precipitation}mm · {current.precipitationProbability}%</span></span><span className="flex min-w-0 items-center gap-1.5 rounded-lg bg-white/80 px-2 py-1.5 ring-1 ring-sky-100/70"><Wind size={13} className="shrink-0 text-cyan-600" /><span className="truncate">바람 {Math.round(current.windSpeed)}km/h</span></span></div>
